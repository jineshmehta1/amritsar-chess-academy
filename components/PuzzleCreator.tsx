...
export function PuzzleCreator({ folderId = "root", existingPuzzle, onBack, batches }: PuzzleCreatorProps) {
      ...
      if (pgnGames.length > 1) {
        ...
        for (let i = 0; i < pgnGames.length; i++) {
          ...
          puzzlesToSave.push({
            ...
            level,
            assignedBatchId: assignedBatch === "All Batches" ? null : batches.find(b => b.name === assignedBatch)?.id || null,
            solutionHint,
            fen: startingFen,
  ...
  const savePuzzle = async () => {
    ...
    const matchedBatch = batches.find((b) => b.name === assignedBatch);
    let payload: any = {
      title: title.trim(),
      description: description.trim(),
      level,
      assignedBatchId: matchedBatch ? matchedBatch.id : null,
      solutionHint,
      folderId: folderId && folderId !== "root" ? folderId : null,
    };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 bg-[#1e1e4f]/40 border border-slate-800 p-6 rounded-3xl h-full min-h-[600px] font-sans">
      <div className="lg:col-span-5 flex flex-col items-center">
        <div
          className={`w-full max-w-[450px] border-4 rounded-3xl shadow-2xl overflow-hidden transition-colors ${
            mode === "RECORD" ? "border-emerald-500/80" : "border-[#FF6B00]/80"
          }`}
        >
          <Chessboard
            position={fen}
        ...
        <div className="mt-4 p-3 bg-slate-950 border border-slate-800 rounded-2xl text-[10px] font-mono w-full break-all text-slate-450 uppercase select-all">
          FEN: {fen}
        </div>
      </div>
      ...
      <div className="lg:col-span-7 flex flex-col gap-6">
        <div className="flex items-center gap-2 border-b border-slate-800 pb-4 justify-between">
          ...
          {mode === "SETUP" && (
            <select
              className="bg-slate-950 text-white border border-slate-800 rounded-xl p-2.5 text-xs font-bold focus:outline-none focus:border-[#FF6B00]"
              value={puzzleSubtype}
          ...
          {existingPuzzle && (
            <button
              ...
              className="flex items-center gap-1.5 text-rose-400 hover:text-rose-350 font-bold text-xs bg-rose-950/30 border border-rose-900/30 px-3 py-1.5 rounded-xl transition-all"
            >
          ...
          {mode === "SETUP" ? (
            <div className="space-y-6">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3">
                <span className="text-[10px] font-bold text-slate-400 uppercase block font-sans">Import PGN Notation</span>
                <textarea
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-xs text-white font-mono outline-none focus:border-[#FF6B00]"
                  rows={3}
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-[#FF6B00] hover:bg-[#E66000] text-white font-bold text-xs rounded-xl shadow-md transition-all"
                >
                  Load & Parse PGN
                </button>
              </div>
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 grid grid-cols-2 gap-4">
                ...
                <div className="bg-orange-950/20 p-3.5 rounded-xl border border-orange-900/30 text-[10px] text-orange-400 flex items-center gap-3">
                  <Star size={16} className="shrink-0 text-[#FF6B00]" />
                  <span>
              ...
              <button
                onClick={toggleMode}
                className="w-full bg-gradient-to-r from-[#FF6B00] to-orange-500 text-white py-3.5 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 shadow-xl hover:brightness-110 transition-all"
              >
            ...
            <div className="space-y-6 animate-in slide-in-from-right-2 duration-300">
              ...
              <div className="space-y-4 pt-4 border-t border-slate-800 text-xs">
                <div>
                  ...
                  <input
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-white font-bold outline-none focus:border-[#FF6B00]"
                ...
                <div>
                  ...
                  <textarea
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 outline-none focus:border-[#FF6B00] text-white"
                    rows={3}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-white font-bold focus:outline-none focus:border-[#FF6B00]"
                    >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-white focus:outline-none focus:border-[#FF6B00]"
                  />
                </div>
                ...
                <div className="flex gap-4">
                  <button
                    onClick={toggleMode}
                    className="px-6 py-3 rounded-xl font-bold text-slate-350 bg-slate-800 hover:bg-slate-700 transition-colors"
                  >
                    Back
                  </button>
                  <button
